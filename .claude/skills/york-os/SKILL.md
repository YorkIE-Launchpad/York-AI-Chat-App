...
| **Drive**           | Docs, Sheets, decks, shared files                                         | MCP: `search_files`, `get_document_content`, `get_spreadsheet_values`, …; write: `create_document`, `update_document_content`, `create_spreadsheet`, `update_spreadsheet_values`, `create_folder` (ask). Prefer `create_spreadsheet` for Google Sheets; use local xlsx only when the user asks for a local `.xlsx` file. |
