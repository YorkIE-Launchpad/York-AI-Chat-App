...
describe('live-assist-question-detect', () => {
  ...
  it('treats ? and clear interrogatives as strong candidates', () => {
    expect(isStrongQuestionCandidate('What is our Q3 revenue?')).toBe(true);
    expect(isStrongQuestionCandidate('Sam: How do we ship this?')).toBe(true);
    expect(isStrongQuestionCandidate('Tell me about the launch')).toBe(true);
    expect(isStrongQuestionCandidate('can you')).toBe(false);
  });
  ...
  it('exports cost guard constants', () => {
    ...
    expect(QUESTION_DEBOUNCE_MS).toBe(800);
