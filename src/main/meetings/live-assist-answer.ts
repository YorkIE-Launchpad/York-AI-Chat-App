...
export type LiveAssistProgressPhase = 'planning' | 'mcp' | 'answering';
...
export function buildLiveAssistAnswerPlanPrompt(
  ...
  const sections = [
    'Plan MCP tool calls to answer a live meeting question with real numbers/facts from company sources.',
    ...
    'If transcript/prep already fully answers and no lookup is needed, return {"calls":[]}.',
...
export function buildLiveAssistAnswerPrompt(
  options: LiveAssistAnswerOptions,
  toolResults: Array<{ tool: string; text: string }> = []
): string {
  const sections = [
    'Answer a live meeting question using the research and context below.',
    'Be concise (at most 6 sentences). Do not invent facts or numbers.',
    'Prefer researched tool results for metrics, statuses, and source facts.',
...
export async function answerLiveAssistQuestion(
  ...
  try {
    ...
    const answerResult = await runPiAiStream(
      buildLiveAssistAnswerPrompt(options, toolResults),
      'Answer concisely in at most 6 sentences. Do not invent facts. Prefer researched numbers and source facts when present.',
      config,
      {
        maxTokens: 256,
        temperature: 0,
